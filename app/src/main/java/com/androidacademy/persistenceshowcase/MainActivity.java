...
public class MainActivity extends AppCompatActivity implements StarWarsDataCallback {
    ...
    private void restoreData() {
        ...
        String[] projection = {
                FilmEntry.COLUMN_NAME_TITLE, FilmEntry.COLUMN_NAME_DESCRIPTION, FilmEntry.COLUMN_NAME_PIC_URL
        };
        Cursor cursor = db.query(FilmEntry.TABLE_NAME, projection, null, null, null, null, null);
        List<Film> filmList = new ArrayList<>();
        if (cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                String title = cursor.getString(cursor.getColumnIndexOrThrow(FilmEntry.COLUMN_NAME_TITLE));
                String description = cursor.getString(cursor.getColumnIndexOrThrow(FilmEntry.COLUMN_NAME_DESCRIPTION));
                String url = cursor.getString(cursor.getColumnIndexOrThrow(FilmEntry.COLUMN_NAME_PIC_URL));
